...
int main(int argv, char* argc[])
{
	if (UG::Create( 1400, 985, false, "Framework Test", 100, 100 ))
	{
		...
		int iPlayerSprite = UG::CreateSprite("./images/sprites.png", 56, 24, true);
		...
		Sprite* device = new Sprite("./images/sprites.png", 56, 24, CVector2(0.5f, 0.5f), CVector4(0.f, 0.f, 1.f, 1.f));
		device->SetLayer(2);
		device->MarkForDraw();
		device->SetPosition(CVector2(iScreenWidth * 0.5f, iScreenHeight * 0.5f));
		device->LoadAnimations("./animations/animations.xml");
		device->SetAnimation("device");		
